...
# ==========================================
# ESPACIOS DE LA BARRA LATERAL (Preparación)
# ==========================================
...
if modo == "Ingreso Manual":
    ...
    df_X_init = pd.DataFrame(np.zeros((num_alt, num_crit)), index=alternativas, columns=criterios)
    df_X = st.data_editor(df_X_init, use_container_width=True)

    # Extraemos los datos listos para procesar
    X_vals = df_X.values
...
# ==========================================
    ...
    st.info("Si deseas que se agreguen automáticamente los Pesos, agrega una segunda hoja a tu archivo Excel con los valores en la columna A. El primer peso será para el criterio 1, el segundo para el 2, etc. Cualquier duda contactar a: [Algún correo]")
    st.info("Si deseas agregar tus Pesos manualmente, sube un archivo con una sola hoja. Luego, en el menú lateral podrás ajustarlos y definir si buscas maximizar (+) o minimizar (-).")
...
# ==========================================
# Esto se ejecuta solo si ya hay datos cargados (ya sea manual o por excel)
if X_vals is not None:
    ...
    for i, col in enumerate(criterios):
        ...
        if pesos_auto is None:
            # Si es manual, usamos tu slider
            w = st.sidebar.slider(f"Peso para {col}", 0.0, 1.0, 0.0, key=f"w_{col}")
        else:
            # Si vienen del excel, los mostramos como info estática
    ...
    # ==========================================
    # BOTÓN Y LÓGICA MATEMÁTICA VIKOR
    # ==========================================
    if st.button("Calcular VIKOR", type="primary"):

        # Convertimos las listas extraídas de la barra lateral a arrays numpy
        ...
        st.write(f"- **Condición C1 (Ventaja Aceptable):** {'✅' if C1 else '❌'}")
        st.write(f"- **Condición C2 (Estabilidad Aceptable):** {'✅' if C2 else '❌'}")
